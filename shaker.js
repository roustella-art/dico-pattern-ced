...
function skBuildStepsUI() {
  ...
  if (!skSteps.length) { list.innerHTML = ''; return; }
  ...
  list.innerHTML = skSteps.map((st, i) => {
    ...
    return `
<div class="sk-step-row ${!st.active ? 'inactive' : ''}">
  <button class="sk-active-toggle ${st.active ? 'on' : ''}" onclick="skToggleStepActive(${i})">${st.active ? '●' : '○'}</button>
  <div class="sk-step-badge">${i+1}</div>
  <div class="sk-step-content">
    ${isRest
      ? `<span class="sk-rest-label">pause</span>`
    ...
    <div class="sk-dir-btns">
      <button onclick="skSetStepDir(${i},'U')" class="${st.dir==='U'?'active-U':''}">↑</button>
      <button onclick="skSetStepDir(${i},'D')" class="${st.dir==='D'?'active-D':''}">↓</button>
    </div>
  ...
  <div class="sk-move-btns">
    <button onclick="skMoveStep(${i},-1)" ${i===0?'disabled':''}>▲</button>
    <button onclick="skMoveStep(${i},+1)" ${i===skSteps.length-1?'disabled':''}>▼</button>
  </div>
  <button class="sk-dup-btn" onclick="skDuplicateStep(${i})" ${atMax?'disabled':''} title="Dupliquer">⧉</button>
  <button class="sk-del-btn" onclick="skRemoveStep(${i})" title="Supprimer">✕</button>
</div>`;
  }).join('');
}
...
/* Preset bar */
.sk-preset-bar { display: flex; gap: 8px; margin-bottom: 16px; align-items: center; }
.sk-preset-select {
...
.sk-preset-btn.save  { border-color: var(--green); color: var(--green); }
.sk-preset-btn.del   { border-color: var(--red); color: var(--red); }
.sk-preset-btn:disabled { opacity: .3; cursor: not-allowed; }
...
.sk-step-row:last-child { border-bottom: none; }
.sk-step-row.inactive .sk-step-content { opacity: 0.28; pointer-events: none; }
.sk-step-badge {
...
.sk-step-content {
  display: flex; align-items: center; gap: 5px; flex: 1; min-width: 0; transition: opacity .15s;
}
.sk-active-toggle {
...
.sk-step-row select {
  ...
  border-radius: 6px; color: var(--text); font-size: 12px; padding: 5px 6px;
  cursor: pointer; outline: none;
}
.sk-str-select   { width: 50px; flex-shrink: 0; padding: 5px 3px; text-align: center; }
.sk-pat-select   { flex: 1; min-width: 0; }
.sk-forme-select { width: 82px; flex-shrink: 0; }
.sk-dur-select   { width: 54px; flex-shrink: 0; font-weight: 600; color: var(--orange); }
.sk-rest-label {
...
.sk-stepper-val {
  font-size: 12px; font-weight: 700; min-width: 16px; text-align: center; color: var(--text);
}
...
<div class="sk-wrap">
  ...
  <div class="sk-preset-bar">
    ...
    <button class="sk-preset-btn save" onclick="skSavePreset()">Sauvegarder</button>
    <button class="sk-preset-btn del" id="sk-delete-preset-btn" onclick="skDeletePreset()" disabled>Supprimer</button>
  </div>
  ...
  <div class="sk-actions">
    <button class="sk-btn sk-btn-play" id="sk-play-btn" onclick="playShaker()" style="${isPlaying?'display:none':''}">▶ Play</button>
    <button class="sk-btn sk-btn-stop" id="sk-stop-btn" onclick="stopShaker()" style="${isPlaying?'':'display:none'}">■ Stop</button>
    <button class="sk-btn sk-btn-clear" onclick="skClearAll()">Reset</button>
  </div>
